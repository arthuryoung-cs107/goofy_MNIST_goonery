...
for mnist_it = 0:(mnist_num-1)
    mnist_it
    mnist_in = readmatrix(['../MNist_csv_raw/MNIST' num2str(mnist_it) '.csv']);

    X00 = mnist_in(2:size(mnist_in, 1),2:size(mnist_in, 2) );
    ...
    X_out = uint8(round(X));
    writematrix(X_out, ['../MNist_csv_50pcentMasked/MNIST' num2str(mnist_it) '_N.csv']);
